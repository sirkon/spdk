...
static *struct nvme_context nvme_context_init() {
    struct nvme_context *res = (*struct nvme_context)malloc(sizeof(struct nvme_context));
    if (res == NULL) {
        perror("nvme_context malloc");
        exit(1);
    }

    res->controllers = TAILQ_HEAD_INITIALIZER(res->controllers);
    res->namespaces = TAILQ_HEAD_INITIALIZER(res->namespaces);
    res->trid = {};
    res->vmd = false;
}

static void nvme_context_cleanup(struct nvme_context *ctx) {
...
static struct spdk_env_opts init_opts() {
...
int init_spdk_env() {
     ...
     *
     */
    struct spdk_env_opts = init_opts();
